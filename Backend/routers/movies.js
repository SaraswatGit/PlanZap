...
movieRouter.put("/updatequote", (req, res) => {
    ...
    db.query("UPDATE user_database SET userquote=? WHERE userid=?", [userquote, userid], (err, result) => {
        if (err) {
            console.log(err);
        }
        else {
...
movieRouter.put("/updatedesc", (req, res) => {
    ...
    const movie_desc = req.body.movie_desc;
    db.query("UPDATE movie_list SET movie_desc=? WHERE id=?", [movie_desc, id], (err, result) => {
        if (err) {
            console.log(err);
        }
        else {
