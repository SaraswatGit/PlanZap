...
diaryRouter.put("/updatediary", (req, res, next) => {
    ...
    db.query("UPDATE diary_table SET data_entry=? WHERE entry_date=? AND userid=?", [data_entry, entry_date, userid], (err, result) => {
        if (err) {
            next(err);
        }
        else {
...
diaryRouter.post("/insertdiary", (req, res) => {
    ...
    db.query("INSERT INTO diary_table (entry_date,data_entry,userid) VALUES (?,?,?)", [entry_date, data_entry, userid], (err, result) => {
        if (err) {
            console.log(err);
        }
        else {
...
diaryRouter.post("/getentry", (req, res) => {
    const userid = req.body.userid;
    const entry_date = req.body.entry_date;
    db.query("SELECT *  FROM diary_table WHERE entry_date=? AND userid=?", [entry_date, userid], (err, result) => {
        if (err) {
            console.log(err);
        }
        else {
